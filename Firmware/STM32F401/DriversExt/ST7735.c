...
void ST7735_Rect(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint16_t color)
{
	ST7735_FillRect(x1, y1, x2, y1, color);
...
void ST7735_PutDec(uint8_t x, uint8_t y, int16_t numb, uint8_t dcount, uint16_t fntColor, uint16_t bkgColor)
{
	...
	for (i = 0; i < 7; i++)
	{
		for (k = 2; k > 0; k--)
		{
			...
			if (k == 2)
				...
				h = 7; // у нижней половины выводим только 6 точек вместо 7
...
void ST7735_PutStr(uint8_t x, uint8_t y, const char str[], uint16_t int8_tColor, uint16_t bkgColor)
{
	while (*str != 0)
	{
